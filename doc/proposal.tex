...
The Rust standard library(std) is a prime example of these idioms in action. Internally, it employs many unsafe operations not only for low level control but also for performance. All public and private API of std are well-documented. Usually a user only needs to interact with the safe API of std.  
