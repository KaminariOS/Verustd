...
% {\footnotesize \textsuperscript{*}Note: Sub-titles are not captured in Xplore andu
% should not be used}
...
The Rust standard library is a prime example of these idioms in action. Internally, it employs many unsafe operations not only for low level control but also for performance. All public and private API of the standard library are well-documented. Usually a user only needs to interact with the safe API of the standard library.  


Because the standard library underpins the entire Rust ecosystem, ensuring its safety and correctness is paramount. Manual auditing by library maintainers, dynamic testing and incomplete static analysis, can reveal the existence of bugs, but insufficient to prove correctness. On the other hand, deductive verification can prove that the implementation refines the specification.           


In the paper, we explore how deductive verification can formally prove the functional correctness of the implementation of the Rust standard library.We take a modular approach: write formal specifications for a module based on its informal documentation and supplement proofs to help the verifier prove the refinement theorem. Specifically, we use the state-of-the-art deductive program verification tool Verus. 
