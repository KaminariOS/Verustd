...
\textbf{Kani}\cite{kani} is a model checker for Rust code. It translates Rust program into an intermediate representation(IR) for verification and then invokes CBMC(C Bounded Model Checker)\cite{kroening2023cbmccboundedmodel}, which checks the IR for property violations via symbolic execution and SAT solving. Kani emphasizes soundness (ensuring that any verified property truly holds in the code) over completeness (meaning it might fail to verify some correct programs). It requires minimal user annotations and enables push-button verification of single‑threaded safety properties. However, it has limited specification expressiveness compared to deductive verification.   
