...
\section{Evaluation}
Our primary metrics for success include the thoroughness of verified invariants, the overall annotation effort, and the degree to which concurrency properties (if explored) can be guaranteed. For correctness coverage, we will focus on essential methods in \texttt{Vec} and \texttt{BinaryHeap}—for example, \texttt{push}, \texttt{pop}, and heap-order maintenance operations.. The notation effort will be gauged by measuring the ratio of proof-related lines to functional code. If we do explore concurrency proofs, we will track how well our specifications hold under concurrent usage and whether any simplified concurrency wrappers can demonstrate Verus’s capabilities.
...
Sometimes the safety of a system depends on a hidden state machine. The code must follow some kind of protocol to be safe. For example, some functions need to be invoked in a specific order, or some MMIO register of a device must be accessed following the specification in the datasheet, or after modifying page table entries the TLB cache needs to be flushed. Out of 240 CVEs of device drivers in the Linux kernel, 72 are due to protocol violation\cite{RFL_security}. Typestate pattern and session types\cite{sessiontype} are proposed for encoding such protocols at zero runtime overhead. The ghost state of Verus is a more general and straightforward approach to keeping track of abstract metadata of the system.       
