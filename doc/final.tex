...
Pure static analysis cannot handle cycles in the call graph or calls to external functions but with a specification-rich function interface we can prove certain unanalyzable runtime behaviors of the code. 
...
Ideally we will have a formally verified Rust compiler like CompCertC so that compiler bugs are eliminated and we only rely on the hardware model. Or at least we need an assembly verifier that proves the generated assembly code preserves the memory safety of Rust source code.   
