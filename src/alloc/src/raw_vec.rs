...
impl<T, A: Allocator> RawVec<T, A> {
    ...
    /// guaranteed.
    #[inline]
    pub(crate) unsafe fn from_raw_parts_in(ptr: PPtr<T>, pt: PointsTo<T>, capacity: usize, alloc: A) -> Self 
    requires usizeNoHighBit(capacity),
    {
        ...
        unsafe {
            ...
            Self {
                inner: RawVecInner::from_raw_parts_in(ptr, pt, capacity, alloc),
                _marker: PhantomData,
            }
        }
    }
...
impl<A: Allocator> RawVecInner<A> {
    ...
    fn try_allocate_in(
        ...
        Ok(Self {
            ...
            cap: Cap::new_verified(capacity),
            alloc,
        })
    }

    #[inline]
    unsafe fn from_raw_parts_in(ptr: PPtr<u8>, pt: PointsTo<u8>, cap: Cap, alloc: A) -> Self {
        Self { ptr, pt, cap, alloc }
    }

    #[inline]
    unsafe fn from_nonnull_in(ptr: PPtr<u8>, pt: PointsTo<u8>, cap: Cap, alloc: A) -> Self 
    requires ptr === pt.pptr()
    {
        // If there is a pointsTo token, ptr cannot be nonull
        Self { ptr, pt, cap, alloc }
    }
    ...
    ) -> Result<(), TryReserveError> 
    {
        ...
        if cap == 0 {
            ...
            // Verus can check for overflow here
            let new_size = elem_layout.size() * cap;
            let ptr = unsafe {
...
proof fn alloc_guard(alloc_size: usize) -> (res: bool)
    ensures !res
{
